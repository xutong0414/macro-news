...
Human feedback is prepared through `inputs/feedback/daily_feedback.example.csv` and rendered as a compact questionnaire in the email. The intended loop is an item-level 1-5 usefulness score and a short comment. This is local preference memory for future ranking/prompt rules, not model fine-tuning.
...
The latest Sunday live email proof completed in 36.10 seconds and used 9,270 Gemini tokens, with estimated LLM cost around $0.0014. During validation tuning, provider/source failures and repair attempts sometimes took around 30-120 seconds, so a production schedule should start the job at least 10-15 minutes before the target inbox time.
