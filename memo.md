...
Live market dashboard rows are fetched asset by asset, with cached real-source rows used before scaffold fallback if a free/public source is temporarily unavailable. The dashboard includes Japan 10Y for USD/JPY yield-spread risk and EUR/USD and USD/JPY for FX coverage, plus notes on extraction time, close/prior basis, Hong Kong morning market-hours caveat, linked data sources, Frankfurter's daily reference-rate convention, and BTC's rolling 24-hour change convention. The run log records which assets were live, cached, or scaffold fallback, and the rendered brief includes a concise Source Status section so the evaluator can see fallback behavior without reading debug logs.
